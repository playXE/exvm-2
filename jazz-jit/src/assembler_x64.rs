...
#[no_mangle]
pub fn new_f32x4(v1: f32, v2: f32, v3: f32, v4: f32) -> f32x4 {
    //println!("{} {} {} {}", v1, v2, v3, v4);
...
impl Assembler {
    ...
    pub fn int_sal(&mut self, mode: MachineMode, dest: Register, lhs: Register, rhs: Register) {
        let x64 = match mode {
            MachineMode::Int32 => 0,
            MachineMode::Int64 => 1,
            _ => unimplemented!(),
...
pub fn xchg(buf: &mut Assembler, x64: u8, src: Register, dest: Register) {
...
#[no_mangle]
pub fn cpuid(buf: &mut Assembler) {
    emit_op(buf, 0x0f);
    emit_op(buf, 0xa2);
}
#[no_mangle]
...
pub fn emit_retq_imm(buf: &mut Assembler, imm: u16) {
    if imm == 0 {
        emit_retq(buf);
    } else {
        emit_op(buf, 0xC2);
...
pub fn emit_sal_reg_cl(buf: &mut Assembler, x64: u8, dest: Register) {
    if dest.msb() != 0 || x64 != 0 {
        emit_rex(buf, x64, 0, 0, dest.msb());
    }
    emit_op(buf, 0xD3);
...
#[no_mangle]
pub fn roundsd(buf: &mut Assembler, dest: XMMRegister, src: XMMRegister, mode: RoundMode) {
    emit_op(buf, 0x66);
    if dest.msb() != 0 || src.msb() != 0 {
        emit_rex(buf, 0, dest.msb(), 0, src.msb());
    }
